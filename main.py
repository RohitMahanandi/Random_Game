...
class Bird:
    ...
    def checking_collision(self, x1, y1, x_position, y_position):
...
while lives > 0:
    ...
    if left_pointed_bird.checking_collision(x1, y1, initial_xposition, initial_yposition) is True or right_pointed_bird.checking_collision(x2,y2,initial_xposition,initial_yposition) is True:
        messagebox.showinfo("Game over", f"Score: {score}")
        print(x1)
        print(x2)
        print(y2)
        print(y1)
        print(initial_xposition)
        print(initial_yposition)
        sys.exit()
